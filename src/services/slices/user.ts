...
>('user/loginUser', async ({ email, password }, { rejectWithValue }) => {
  ...
  if (!response?.success) {
    return rejectWithValue('Во время авторизации произошла ошибка');
  }
...
>('user/registerUser', async (creds, { rejectWithValue }) => {
  ...
  if (!response.success) {
    return rejectWithValue('Во время регистрации произошла ошибка');
  }
...
export const logoutUser = createAsyncThunk<void, void, { rejectValue: string }>(
  ...
  async (_, { rejectWithValue }) => {
    ...
    if (!response.success) {
      return rejectWithValue('При выходе из аккаунта произошла ошибка');
    }
    ...
    builder
      ...
      .addCase(logoutUser.rejected, (state, action) => {
        ...
        state.loginUserError =
          action.payload ??
          action.error.message ??
          'Произошла ошибка во время выхода из аккаунта';
      });
  }
});
